# Commercial License Terms

Copyright (c) 2026 OmniDev Corp. All rights reserved.

These terms are a baseline commercial license template for paid OmniBot
customers. They apply only when the copyright holder grants a written
commercial license, invoice, order form, contract, or other written permission
that references this file or clearly grants commercial rights.

## 1. License Grant

After purchase and subject to the paid agreement, the customer receives a
non-exclusive, non-transferable license to use, run, modify, and integrate
OmniBot for the licensed server, project, organization, or deployment scope.

## 2. Permitted Use

Unless the paid agreement says otherwise, the customer may:

- run OmniBot in private, internal, production, or hosted environments;
- modify OmniBot for their own licensed use;
- integrate OmniBot with Discord infrastructure, dashboards, AI Moderator,
  proxy services, databases, and related systems;
- create private forks and private derivative works for the licensed scope.

## 3. Restrictions

Unless the paid agreement expressly allows it, the customer may not:

- publish or distribute the original source code;
- sell, sublicense, rent, lease, or share the source code as a standalone
  product;
- make the repository public;
- provide the source code to third parties outside the licensed organization;
- remove copyright notices, license text, attribution, or authorship notices;
- use the Software outside the purchased license scope.

## 4. Ownership

All right, title, and interest in and to the original Software, including all
associated intellectual property rights, remain vested exclusively in OmniDev
Corp.

Customer modifications created under a paid license may be owned by the customer
unless the paid agreement states otherwise, but those modifications do not remove
the author's ownership of the original Software.

## 5. White-Label, Resale, And SaaS Rights

White-label distribution, resale, sublicensing, marketplace distribution,
public SaaS use, or source-code redistribution require explicit written
permission in the paid agreement.

## 6. No Warranty

THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR
IMPLIED, INCLUDING BUT NOT LIMITED TO WARRANTIES OF MERCHANTABILITY, FITNESS FOR
A PARTICULAR PURPOSE, NON-INFRINGEMENT, SECURITY, RELIABILITY, OR AVAILABILITY.

## 7. Conflict

If a signed contract, invoice, order form, or written agreement conflicts with
this file, the signed or expressly approved agreement controls.
